...
- The web UI (`src/shufflebase/web/`) is hand-written vanilla JS/CSS served as static files by FastAPI, not a separate Node build — this keeps `pip install -e ".[web]"` + `shufflebase serve` a single self-hosted command with no build step. `cli.py`'s `serve` command imports it lazily so the CLI works without the `web` extra installed.
- The project is not published to PyPI (`release.yml` only attaches build artifacts to a GitHub Release, no `twine upload`/trusted-publish step) — README's Install section is source-only until that's wired up. The default branch is `master`, not `main`; keep workflow triggers, doc links, and branch references pointed at `master`.
